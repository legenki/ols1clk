...
DATABASENAME=wp_clilk
USERNAME=usrd2sgG4Ba

WORDPRESSPATH=$SERVER_ROOT/clilk.com
WPPORT=80
SSLWPPORT=443
...
{
    if [ -e "$SERVER_ROOT/conf/httpd_config.conf" ] ; then
        cat $SERVER_ROOT/conf/httpd_config.conf | grep "virtualhost clilk.com" >/dev/null
        if [ $? != 0 ] ; then
            sed -i -e "s/adminEmails/adminEmails $EMAIL\n#adminEmails/" "$SERVER_ROOT/conf/httpd_config.conf"
            sed -i -e "s/ls_enabled/ls_enabled   1\n#/" "$SERVER_ROOT/conf/httpd_config.conf"

            VHOSTCONF=$SERVER_ROOT/conf/vhosts/clilk.com/vhconf.conf

            cat >> $SERVER_ROOT/conf/httpd_config.conf <<END 

virtualhost clilk.com {
...
address                 *:$WPPORT
secure                  0
map                     clilk.com $SITEDOMAIN
}


listener clilk.comssl {
address                 *:$SSLWPPORT
secure                  1
map                     clilk.com $SITEDOMAIN
keyFile                 $SERVER_ROOT/conf/$KEY
...
END
            ...
            mkdir -p $SERVER_ROOT/conf/vhosts/clilk.com/
            cat > $VHOSTCONF <<END 
docRoot                   \$VH_ROOT/
index  {
  useServer               0
